...
for my $file (@file){
open(INPUT2,"$file");
my $outputnew = "$sort_rna"."_tir_clean";
...
my ($chr,$start,$end) = ($info2[1],$start[0],$start[1]);
my $name = "$info2[0]:$info2[1]:$start[0]:$start[1]:$start[2]";
my $internal_srna = get_rna_count($name,$chr,$start,$end,$sort_rna);
print OUTPUT1 "$name\t$internal_srna\n";
my ($chr,$start,$end) = ($info2[1],$end[0],$end[1]);
my $name = "$info2[0]:$info2[1]:$end[0]:$end[1]:$end[2]";
my $internal_srna = get_rna_count($name,$chr,$start,$end,$sort_rna);
print OUTPUT1 "$name\t$internal_srna\n";
...
`awk '{if(\$2 == "$chr"){print ;}}' $file | sort -k 3,3n -k 4,4n > sort_coor_new10_tir`; 
open(INPUT1,"sort_coor_new10_tir");
my $exp_flag=0;
my $exp_str="";
my $mule_overlap=0;
my $exon_overlap=0;
